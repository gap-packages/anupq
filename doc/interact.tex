...
`IsCapable' are set for the group returned by `Pq'  (see~"Properties  and
Attributes").
...
`PqEpimorphism' (see~"Properties and Attributes").
...
`StandardPresentation' (see~"Properties and Attributes").
...
`PqDescendants' (see~"Properties and Attributes").
...
*Notes:*
The  attributes  and  property  `NuclearRank',  `MultiplicatorRank'   and
`IsCapable'  are  set  for  the  group   returned   by   `PqCurrentGroup'
(see~"Properties and Attributes").

In order to set the above attributes  and  property  the  next  class  is
calculated after which the class is reverted to the previous class.  Thus
`PqCurrentGroup(<i>);'    cannot    be    immediately     followed     by
`PqRevertToPreviousClass(<i>);' (see~"PqRevertToPreviousClass").
