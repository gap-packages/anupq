...
*Notes*: 
For technical reasons, interactive {\ANUPQ} process  are  never  assigned
prime identification indices <i>.
...
<p>-quotient of the group <F> of the process, specified by  the  argument
<p> (a prime) or <Q> (a pc <p>-group), if given, and <options>.  must  be
an fp or pc group (<F> should have been  given  (as  first  argument)  to
`PqStart' to start the interactive {\ANUPQ} process; see~"PqStart"),  and
must be an fp or pc group. It is only permissible to  omit  the  argument
<p> or <Q>, in the case where <F> is a pc group and it *knows* that it is
a <p>-group.

If the user supplies a pc group <Q> which must be a $p$-quotient of  <F>,
the  package  {\AutPGrp}  is  called   automatically   to   compute   the
automorphism group of <Q>; an error will occur  that  asks  the  user  to
...
separate calls to the `pq' program, we now show how, with the  interactive
...
4
gap> K := Pq( 4 : Prime := 2, ClassBound := 1 ); #`pq' process no. is 4
<pc group of size 4 with 2 generators>
gap> StandardPresentation( 4, K : ClassBound := 14 );
...
something that required two separate calls to the `pq' program can now  be
...
6
gap> G := Pq( 6 : Prime := 3, ClassBound := 3 );
<pc group of size 729 with 6 generators>
gap> phi := EpimorphismStandardPresentation( 6, 3 : ClassBound := 3 );
...
gap> #... its id. no. is 8 (the 4th non-prime number)
gap> des := PqDescendants( 8 : OrderBound := 6, ClassBound := 5 );;
gap> Length(des);
83
gap> List(des, Size);
...
9
gap> des := PqDescendants( 9 : OrderBound := 3, ClassBound := 2,
>                              CapableDescendants );
...
10
gap> des := PqDescendants( 10 : Metabelian, ClassBound := 3,
...
`Exponent',  `Relators',  `GroupName',  `Metabelian',  `Identities'   and
`OutputLevel' (see Chapter~"ANUPQ options" for a detailed description for
these options). The option `Prime' is required unless already provided to
`PqStart'.
...
program to do a collection on <word>, a string representing the product of
...
for the <i>th or default interactive {\ANUPQ} process, direct  the  `pq'
program to compute the left normed commutator of the list <words> of words
...
for the <i>th or default interactive {\ANUPQ} process,  direct  the  `pq'
program to compute the isomorphism mapping from the pc  $p$-group  of  the
