...
finish the options `Prime' and `Classbound', in particular, will still be
on the `OptionsStack':
...
We altered `OnBreak' (see~"ref:OnBreak" in the Reference manual) to reduce
the back-tracing on entry into a break loop.

\beginexample
gap> OnBreak := Where;; # Restore `OnBreak' to its usual value
\endexample
...
There are some other strategies which may have helped us  see  our  error
...
provides information on each class as it is generated:
...
After seeing the information on class 2 we would have got the  idea  that
the `Classbound' option  was  not  recognised  and  would  probably  have
