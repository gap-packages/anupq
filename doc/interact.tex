...
those  zombie  processes  (e.g.~by  piping  the output  of  a  `ps'  with
...
using the  function~"PqRestorePcPresentation".  If `PqStart'  is given an
integer argument <workspace>, then the  `pq' program is started up with a
workspace  (an  integer  array)  of  size <workspace>  (i.e.   $4  \times
<workspace>$ bytes in a  32-bit environment); otherwise, the `pq' program
...
for the  <i>th or  default interactive {\ANUPQ}  process, returns  a pair
`[<p>,<n>]'     where    <n>    is     the    number     of    generators
(see~"PqNrPcGenerators")  in the  present  pc presentation  of the  group
currently determined by the process.

If this presentation is consistent, then $p^n$ is the order of the group.
Otherwise (if tails have been added but the necessary consistency checks,
relation  collections,  exponent   law  checks  and  redundant  generator
eliminations have  not yet been  done), $p^n$ is  an upper bound  for the
order of the group.

\>PqOrder( <i> ) F
\>PqOrder() F

for  the <i>th  or default  interactive {\ANUPQ}  process,  returns $p^n$
...
there   is   no    {\GAP}   pc   group   returned;   see~`PqCurrentGroup'
("PqCurrentGroup") if you need the corresponding {\GAP} pc group.
...
to the  function~"PqPCover", this  function does not  return a  {\GAP} pc
group.
...
in the pc generators.  For  the representation of these words compare the
description of the function~"PqCollect".
...
for  the   <i>th  or  default  interactive  {\ANUPQ}   process,  takes  a
user-defined word in the defining generators of the original presentation
of the fp or pc group of  the process.  Each generator is mapped into the
current pc presentation, and the resulting word is collected with respect
to the current pc presentation.  The result of the collection is returned
as a list of generator number, exponent pairs.
...
for  the <i>th  or default  interactive  {\ANUPQ} process,  takes a  list
...
mapped into the current pc presentation.  The list <words> is interpreted
as a  left-normed commutator  and this is  collected with respect  to the
current pc presentation.   The result of the collection  is returned as a
list of generator number, exponent pairs.
...
automorphism in following way.  The rows of each matrix correspond to the
pc generators  of weight  one.  Each  row is the  exponent vector  of the
image  of the  corresponding weight  one generator  under  the respective
automorphism.
...
$p$-quotient of  the previous  class to the  $p$-quotient of  the present
class.  You may wish to set the `InfoLevel' of `InfoANUPQ' to 2 (or more)
in order to see the output from the `pq' (see~"InfoANUPQ").
