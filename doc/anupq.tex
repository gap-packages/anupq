...
function.  Below  we  list  the   options   recognised   by   `Pq'   (see
...
The  following   options   are   recognised   by   `PqDescendants'   (see
Chapter~"ANUPQ options" for detailed descriptions).
...
*Note:*
In  general,  the   user   should   have   no   need   for   the   option
`PcgsAutomorphisms', so long as the {\GAP} share  package  {\AutPGrp}  is
installed. If the package {\AutPGrp} is installed it is used  to  compute
the automorphism group of <G> and then `PcgsAutomorphisms' is by  default
set to the value  of  `IsSolvable(  AutomorphismGroup(  <G>  )'.  If  the
package {\AutPGrp} is *not* installed and <G> does *not*  have  $p$-class
1, then the automorphism group of <G> must be known, since  the  inferior
{\GAP} library `AutomorphismGroup' command will be used.

If  the  automorphism  group  of  <G>  is  insoluble  or  if  the  option
`PcgsAutomorphisms' is set to `false', then the  `pq'  binary  will  call
{\GAP} together with the {\AutPGrp} package for certain  orbit-stabilizer
calculations. (So, in any case, one should ensure the {\AutPGrp}  package
is installed.)
