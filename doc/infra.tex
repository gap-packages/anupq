...
used the `GroupName' option (see e.g.~"Pq") this  will  be  the  default:
`"G"');
...
\quad`order'& the order of the largest lower exponent-$p$  central  class
found the group (usually `group') whose pc presentation is defined  by  a
call to the `pq' binary;

\quad`pcoverclass'& the class of the  $p$-covering  group  last  computed
(determined from `pq' binary output);

\quad`pcoverorder'& the order of the  $p$-covering  group  last  computed
(determined from `pq' binary output);
