...
1' displays minimal output and `OutputLevel's for <n>  greater  than  `1'
give progressively more output. Non-interactively,  setting  this  option
implies `Verbose'.
...
displayed, and if `InfoLevel' of `InfoANUPQ' is 3 each line of the  input
script to the `pq' binary is  `Info'-ed  behind  a  ```ToPQ> '''  prompt.
...
of `InfoANUPQ' described in Section~"InfoANUPQ".
...
<options> are as for `Pq' (see~"Pq"). As for `Pq', `PqEpimorphism' may be
called with no arguments or one integer argument, in  which  case  it  is
being used interactively (see~"PqEpimorphism!interactive");  and  also  as
for  `Pq',  the  same  options  may  be  used,  except  that   `Verbose',
`SetupFile'  and  `PqWorkspace'  are  ignored  by  the  interactive  `Pq'
function. The  same  alternative  methods  of  passing  options  for  the
non-interactive `Pq' function are also available for the  non-interactive
