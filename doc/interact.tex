...
return, for  the  finitely  presented  group  of  the  <i>th  or  default
interactive {\ANUPQ} process (let us call it <F>), the $p$-covering group
of the $p$-quotient of  <F>  specified  by  <options>,  as  a  pc  group,
i.e.~the group  returned  by  `PqPCover(  <i>  :  <options>  );'  is  the
$p$-covering group of the $p$-quotient `Pq( <i> : <options> )'. Thus  the
arguments and options that the interactive `PqPCover' accepts are exactly
those expected for the interactive  `Pq'  (see~"Pq!interactive"),  except
that if the $p$-quotient  `Pq(  <i>  :  <options>  )'  has  already  been
computed, the $p$-cover of that $p$-quotient is returned irrespective  of
any options that may have been supplied.
