...
\quad`class'& the largest lower exponent-$p$ central class found for  the
group (usually `group') whose pc presentation is defined by a call to the
`pq' binary;

\quad`forder'& the factored order of the factor group  of  largest  lower
exponent-$p$ central class found for the group (usually `group') whose pc
presentation is defined by a call to the `pq' binary (this factored order
is given as a list `[$p$, $n$]', indicating an order of $p^n$);

\quad`pcoverclass'& the lower exponent-$p$ central class  found  for  the
$p$-covering group of a $p$-quotient of the group (usually `group') whose
pc presentation is defined by a call to the `pq' binary;
...
of words in the generators of  a  free  of  fp  group)  the  left  normed
