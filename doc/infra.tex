...
\item{$\bullet$} 
if you passed options to a function and ran into an error  then  you  may
need to type `ResetOptionsStack();' so that those options are  not  still
on the `OptionsStack' to affect  subsequent  functions  and  setting  the
`ANUPQWarnOfOtherOptions := true;' (particularly useful  for  the  novice
user) will warn you of misspelt options and may help in diagnosing  other
option  usage  problems  (details  are  in  Section~"Hints  and  Warnings
