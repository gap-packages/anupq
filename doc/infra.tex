...
converts strings representing relators from  a  format  the  `pq'  binary
...
\index{banner}
To use the {\ANUPQ} Package you have to request it  explicitly.  This  is
done by calling
...
(the same as those passed to the `pq' binary);

\quad`rels'& a list of the relators of the group `group' as strings  (the
same as those passed to the `pq' binary);

\quad`name'& the name of the group whose pc presentation is defined by  a
call to the `pq' binary (according to the `pq' binary -- unless you  have
...
group of the group (usually `group') found by a call to the `pq' binary;
...
call to the `pq' binary (this factored order is given as  a  list  `[$p$,
...
\quad`pQpcp' & if bound, this field is a record  containing  the  options
used the last time the `pq' computed a pc presentation for a $p$-quotient
of `group' i.e.~if it is bound then there has been such  a  call  (it  is
used mainly to avoid re-computing a  pc  presentation  that's  been  done
already, by functions that require that a pc presentation is known to the
`pq' binary);

\quad`pQuotient'   &   is   set   to   the   value   returned   by   `Pq'
...
\quad`SPpcp' & if bound, this field is a record  containing  the  options
used the last time the `pq' computed a pc presentation for a $p$-quotient
of `group' i.e.~if it is bound then there has been such  a  call  (it  is
used mainly to avoid re-computing a  pc  presentation  that's  been  done
already, by functions that require that a pc presentation is known to the
...
want to see and output from the `pq' binary influenced by  the  value  of
...
enables the display of timing and memory usage data from the `pq' binary.
...
that was not directly invoked by the user (some commands  require  {\GAP}
to discover something about the current state known to the `pq'  binary).
In some cases, the `pq' binary produces a lot of output despite the  fact
that the `OutputLevel' (see~"option OutputLevel") is unset or is  set  to
0; such output except (for timing and memory usage) is also `Info'-ed  at
`InfoANUPQ' level 3.
...
enables the display of all the commands  directed  to  the  `pq'  binary,
behind a ```ToPQ> ''' prompt (so that you can  distinguish  it  from  the
output from the `pq' binary).

\beginexample
gap> SetInfoLevel(InfoANUPQ, 5);
\endexample

enables the display of the  `pq'  binary's  banner,  prompts  for  input.
Finally,
...
\index{menu item!of pq binary}
\index{option!of pq binary is a menu item}
*Note:*
By ``options'' we refer to {\GAP} options. The `pq' binary also uses  the
term ``option''; to distinguish the two usages  of  ``option'',  in  this
manual we use the term *menu item* to  refer  to  what  the  `pq'  binary
...
group <group> prepared as a list of relators for the `pq' binary.
...
even though the `pq' binary accepts relations,  all  elements  of  <rels>
