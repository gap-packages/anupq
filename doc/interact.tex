...
from the following list should be given, separated by commas like  record
components (see~"ref:function call with options"). This version  of  `Pq'
performs the same function as its non-interactive counterpart and  except
that it doesn't recognise the `SetupFile' or `Verbose' option it uses the
same options (see~"Pq"). The following is one of the examples for the non-interactive
`Pq' redone with the interactive version:
